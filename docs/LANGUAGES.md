...
Command names remain compatible: `!bang`, `!shop`, `!shop info <id>`, `!duckstats`, `!inventory`,
`!duckrank hits`, `!myrank`, `!duckhelp`, `!duckplanning`, Owner
`!bread`/`!duckcall`, `.duckplanning next` and `.top [xp|hits]` are the same
